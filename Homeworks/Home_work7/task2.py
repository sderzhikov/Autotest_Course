...
class PersonInfo:
    ...
    def new_salary(self):
        ...
        top_letters = sorted(letter_count, key=lambda x: letter_count[x], reverse=True)[:3]
        sum_letters = sum(letter_count[letter] for letter in top_letters)
        res = 1337 * self.age * sum_letters
        return res
...
for i, method in enumerate(data):
    assert_error = f'Не прошла проверка для метода {method.__qualname__} экземпляра с атрибутами {method.__self__.__dict__}'
    assert method() == test_data[i], assert_error
    print(f'Набор для метода {method.__qualname__} экземпляра класса с атрибутами {method.__self__.__dict__} прошёл проверку')
print('Всё ок')
